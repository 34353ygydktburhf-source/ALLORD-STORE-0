...
## 👥 5. نظام المجتمع والتواصل (Community & Messaging)

يحتوي الموقع على نظام تواصل متطور (SaaS-Grade) يعتمد على عزل المحادثات حسب السياق:

*   **عزل المحادثات (Chat Isolation):** يتم فرز المحادثات بناءً على `postId`. هذا يعني أن المشتري والبائع يمكن أن يمتلكا "شات خاص" لكل إعلان منفصل، مما يمنع اختلاط المواضيع.
*   **نظام التشفير والوساطة:**
    - يتم تشفير الروابط والأرقام تلقائياً ويمنع ظهورها إلا بعد تدخل "الوسيط" (Admin).
    - **موقع الكود:** `CommunityContext.tsx` يحتوي على دوال `sendMessage` و `approveMessage`.
    - عند طلب وسيط، يتم إرسال إشعار للوحة المطور (`DeveloperDashboard.tsx`) للموافقة على كشف البيانات أو مراقبة العملية.
*   **التطوير المستقبلي:** الربط بقاعدة بيانات مثل `Firestore` سيتطلب تغيير الـ Context ليقوم بعمل `onSnapshot` للاستماع للرسائل الجديدة فورياً.

---

## 📊 6. لوحة التحكم والملف الشخصي (Profile & Dashboard)

تم توفير صفحة `/profile` لإدارة الحساب:

*   **حفظ الحسابات:** يمكن للمستخدم حفظ الـ IDs الخاصة به في الألعاب المختلفة (ببجي، فري فاير...) ليتم استدعاؤها فوراً عند الشراء.
*   **سجل الطلبات:** يعرض حالة الطلبات (مكتمل، جاري، بانتظار إثبات الدفع).
*   **طريقة الحفظ:** حالياً نستخدم `localStorage` و `LoginContext`. للربط الحقيقي، يجب ربط الـ IDs بجدول `user_accounts` في قاعدة البيانات.
